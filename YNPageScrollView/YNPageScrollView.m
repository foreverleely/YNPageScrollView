...
@property (nonatomic, strong) YNPageTableView           *mainTableView;
@property (nonatomic, strong) YNPageListContainerView   *listContainerView;
// 当前滑动的listView
@property (nonatomic, weak) UIScrollView                *currentListScrollView;
@property (nonatomic, strong) NSMutableDictionary <NSNumber *, id<YNPageListViewDelegate>> *validListDict;
...
- (void)initSubviews {
    ...
    self.mainTableView = [[YNPageTableView alloc] initWithFrame:CGRectZero style:UITableViewStylePlain];
    ...
    if ([self shouldLazyLoadListView]) {
        self.listContainerView = [[YNPageListContainerView alloc] initWithDelegate:self];
...
- (void)reloadData {
    ...
    for (id<YNPageListViewDelegate> list in self.validListDict.allValues) {
...
- (void)mainScrollViewOffsetFixed {
    ...
    if ([self shouldLazyLoadListView]) {
        for (id<YNPageListViewDelegate> listItem in self.validListDict.allValues) {
...
- (UIView *)listContainerView:(YNPageListContainerView *)listContainerView listViewInRow:(NSInteger)row {
    id<YNPageListViewDelegate> list = self.validListDict[@(row)];
    if (list == nil) {
